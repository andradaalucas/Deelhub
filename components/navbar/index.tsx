				...
				<div className='hidden h-[60px] items-center border-b px-6 lg:flex'>
					...
					<DropdownMenu>
						<DropdownMenuTrigger asChild>
							<Button className='relative ml-auto h-8 w-8 border bg-white hover:bg-gray-100/40 ' size='icon' variant='ghost'>
								<BellIcon className='h-4 w-4' />
								<span className='sr-only'>Toggle notifications</span>
								{notificationsList.filter((notification) => notification.active).length > 0 && (
						...
						<DropdownMenuContent align='start'>
							...
							{notificationsList.map((notification, index) => (
								<DropdownMenuItem key={index} onClick={() => confirmNotification(notification.id)}>
									<span
